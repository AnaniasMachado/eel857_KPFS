...
for scenario in scenarios
    for type in types
        for size in sizes
            ...
            solutions_folder = "../../solutions/Tabu_Search/scenario$(scenario)/$(type)$(scenario)/$(size)/"
            if (scenario == 3) && (type == "correlated_sc")
                continue
            elseif (scenario == 3) && (type == "fully_correlated_sc")
                continue
            elseif (scenario == 3) && (type == "not_correlated_sc") && (size in [300, 500])
                continue
            elseif (scenario == 3) && (type == "not_correlated_sc") && (size == 700)
                for idx in 2:10
                    try
                        ...
                        solution_filename = "grb_ts_kpfs_$(idx)"
